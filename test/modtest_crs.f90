...
subroutine collect_crs(testsuite)
  ...
  testsuite = [ &
    ...
    , new_unittest("crs xAx", test_xAx_crs) &
    , new_unittest("crs xAy", test_xAy_crs) &
    , new_unittest("crs traceproduct", test_traceproduct_crs) &
...
subroutine test_xAx_crs(error)
  ...
  call check(error, abs(quad - expected) < tol, more = 'crs xAx fixed')
  if(allocated(error))return
  ...
  expected = dot_product(x, matmul(mat, x))
  tol = 1.e-3_wp * max(abs(expected), 1._wp)
  call check(error, abs(quad - expected) < tol, more = 'crs xAx random')
  if(allocated(error))return

  !empty matrix -> 0
  coo = coosparse(4, unlog = sparse_unit)
  crs = coo
  call check(error, abs(crs%xAx([1._wp, 2._wp, -1._wp, 3._wp])) < tol_wp&
           , more = 'crs xAx empty')

end subroutine test_xAx_crs

!BILINEAR FORM
subroutine test_xAy_crs(error)
  type(error_type), allocatable, intent(out) :: error
  ...
  tol = 1.e-3_wp * max(abs(expected), 1._wp)
  call check(error, abs(bilin - expected) < tol, more = 'crs xAy fixed')
  if(allocated(error))return

  !identity: xAx(x) == xAy(x, x)
  call check(error, abs(crs%xAx(x) - crs%xAy(x, x)) < tol_wp&
           , more = 'crs xAx identity xAy(x,x)')
  if(allocated(error))return
  ...
  expected = dot_product(x, matmul(mat, y))
  tol = 1.e-3_wp * max(abs(expected), 1._wp)
  call check(error, abs(bilin - expected) < tol, more = 'crs xAy random')
  if(allocated(error))return

  !CRS result equals COO result for the same input (before reassigning the objects)
  !different accumulation order -> allow a small relative tolerance
  call check(error, abs(crs%xAy(x, y) - coo%xAy(x, y)) < tol_wp * max(abs(crs%xAy(x, y)), 1._wp)&
           , more = 'crs-xAy close to coo')
  if(allocated(error))return

  !empty matrix -> 0
  coo = coosparse(4, unlog = sparse_unit)
  crs = coo
  call check(error, abs(crs%xAy([1._wp, 2._wp, -1._wp, 3._wp], [3._wp, -1._wp, 2._wp, 1._wp])) < tol_wp&
           , more = 'crs xAy empty')

end subroutine test_xAy_crs

!TRACE OF A BLOCK PRODUCT
subroutine test_traceproduct_crs(error)
  type(error_type), allocatable, intent(out) :: error
  ...
  tol = 1.e-3_wp * max(abs(expected), 1._wp)
  call check(error, abs(result - expected) < tol, more = 'crs traceproduct whole I')
  if(allocated(error))return
  ...
  result = crs%traceproduct(1, n, 1, n, b)
  tol = 1.e-3_wp * max(abs(expected), 1._wp)
  call check(error, abs(result - expected) < tol, more = 'crs traceproduct whole dense')
  if(allocated(error))return
  ...
  result = crs%traceproduct(1, 2, 1, 2, b(1:2, 1:2))
  tol = 1.e-3_wp * max(abs(expected), 1._wp)
  call check(error, abs(result - expected) < tol, more = 'crs traceproduct 2x2 diag')
  if(allocated(error))return

  !Case 4: 1x1 off-diagonal at (1,3): A(1,3)=13
  expected = mat(1,3) * 7._wp
  result = crs%traceproduct(1, 1, 3, 3, reshape([7._wp], [1,1]))
  tol = 1.e-3_wp * max(abs(expected), 1._wp)
  call check(error, abs(result - expected) < tol, more = 'crs traceproduct 1x1 off')
  if(allocated(error))return
  ...
  do id = 1, 4
    b(id, id) = 1._wp
  enddo
  call check(error, crs%traceproduct(1, 3, 1, 3, b(1:3, 1:3)) == &
                  coo%traceproduct(1, 3, 1, 3, b(1:3, 1:3)) &
           , more = 'crs-traceproduct equal to coo')

end subroutine test_traceproduct_crs
