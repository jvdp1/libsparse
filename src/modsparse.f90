...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!GEN!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!aaa
 ...
 type,abstract::gen_sparse
  ...
  integer(kind=int64),allocatable::perm_64(:)  !Ap(i,:)=A(perm(i),:)
  ...
  procedure :: setpermutation_32
  procedure :: setpermutation_64
  generic,public::setpermutation => setpermutation_32, setpermutation_64
  procedure::setsorted
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!CRS_64!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!aaa
 ...
 type,extends(gen_sparse)::crssparse_64
  private
  ...
  private
  !> @brief Adds the value val to mat(row,col); e.g., call mat\%add(row,col,val)
  procedure,public::add=>add_crs_64
!#if (_SPAINV==1)
!  !> @brief Computes and replaces the sparse matrix by the (complete) Cholesky factor
!  procedure,public::chol=>getchol_crs_64
!#endif
  !> @brief Deallocates the sparse matrix and sets to default values
  procedure,public::destroy=>destroy_crs_64
!  procedure::diag_vect_crs_64
!  procedure::diag_mat_crs_64
!  !> @brief Gets the (upper) diagonal elements of a matrix; e.g., array=mat%diag()
!  !! OR mat=mat%diag(10) (to extract the diagonal + 10 off-diagonals)
!  generic,public::diag=>diag_vect_crs_64,diag_mat_crs_64
  !> @brief Returns the value of mat(row,col); e.g., ...=mat\%get(row,col)
  procedure,public::get=>get_crs_64
!#if (_SPAINV==1)
!  !> @brief Computes and replaces the sparse matrix by the (complete) LDLt (L is stored in the upper triangle and D in the diagonal)
!  procedure,public::getldlt=>getldlt_crs_64
!#endif
  !> @brief Gets memory used
  procedure,public::getmem=>getmem_crs_64
!  !> @brief Initializes the vectors ia,ja,and a from external vectors
!  procedure,public::external=>external_crs_64
  !> @brief Get function for the internal vector ia of row pointers
  procedure,public::get_rowptr=>get_rowptr_crs_64
  !> @brief Get function for the internal vector ja of column values
  procedure,public::get_colval=>get_colval_crs_64
  !> @brief Get function for the internal vector a of non-zero values
  procedure,public::get_nzval=>get_nzval_crs_64
!  !> @brief Get diagonal elements of an approximate inverse using Harville (1999)
!  procedure,public::harville=>harville_crs_64
!#if (_SPAINV==1)
!  !> @brief Computes and replaces the sparse matrix by an incomplete Cholesky factor
...
!  !> @brief Solver using LDLt decomposition
!  procedure,public::solveldlt=>solveldlt_crs_64
  !> @brief Multiplication with a vector
  procedure::multbyv=>multgenv_csr_64
  !> @brief Multiplication with a matrix
  procedure::multbym=>multgenm_csr_64
  !> @brief Returns the number of non-zero elements
  procedure,public::nonzero=>totalnumberofelements_crs_64
!#if (_METIS==1)
!  !> @brief Returns the ordering array obtained from METIS
!  procedure,public::getordering=>getordering_crs_64
!#endif
!  !> @brief Releases Pardiso memory if possible
#if (_PARDISO==1)
  procedure,public::resetpardiso=>reset_pardiso_memory_crs_64
#endif
  !> @brief Prints the sparse matrix to the output sparse\%unlog
  procedure,public::print=>print_crs_64
  !> @brief Prints the sparse matrix in a rectangular/square format to the default output
  procedure,public::printsquare=>printsquare_crs_64
!  !> @brief Saves the matrix (internal format) to stream file
!  procedure,public::save=>save_crs_64
  !> @brief Scales all entries of mat by real scalar val; e.g., call mat\%scale(val)
  procedure,public::scale=>scale_crs_64
!  !> @brief Sets an entry to a certain value (even if equal to 0); condition: the entry must exist; e.g., call mat\%set(row,col,val)
!  procedure,public::set=>set_crs_64
  !> @brief MKL PARDISO solver
  procedure,private::solve_crs_64_vector
  procedure,private::solve_crs_64_array
  generic,public::solve=>solve_crs_64_vector,solve_crs_64_array
  !> @brief Sorts the elements in a ascending order within a row
  procedure,public::sort=>sort_crs_64
!#if (_SPAINV==1)
!  !> @brief Computes and replaces by the sparse inverse
!  procedure,public::spainv=>getspainv_crs_64
!#endif
!  !> @brief Gets a submatrix from a sparse matrix
!  procedure,public::submatrix=>submatrix_crs_64
  final::deallocate_scal_crs_64,deallocate_rank1_crs_64
 end type
...
!  !**DIAGONAL ELEMENTS
!  module function diag_vect_crs_64(sparse) result(array)
!   class(crssparse_64),intent(inout)::sparse
!   real(kind=wp),allocatable::array(:)
!  end function
  !**ADD ELEMENTS
...
!  !**EXTERNAL
!  module subroutine external_crs_64(sparse,ia,ja,a)
!   class(crssparse_64),intent(inout)::sparse
!   integer(kind=int32),intent(in)::ia(:),ja(:)
!   real(kind=wp),intent(in)::a(:)
!  end subroutine
!  !**HARVILLE
!  module subroutine harville_crs_64(sparse, ngibbs, nburn, diaginv, seed)
!   class(crssparse_64), intent(inout)::sparse
!   integer, intent(in) :: ngibbs, nburn
!   real(wp), intent(out), allocatable :: diaginv(:)
!   integer, intent(in), optional :: seed
...
!#if (_SPAINV==1)
!  !**GET (COMPLETE) CHOLESKY FACTOR
!  module subroutine getchol_crs_64(sparse,minsizenode)
!   class(crssparse_64),intent(inout)::sparse
!   integer(kind=int32),intent(in),optional::minsizenode
!  end subroutine
!  !**GET LDLt DECOMPOSITION
!  module subroutine getldlt_crs_64(sparse,minsizenode)
!   class(crssparse_64),intent(inout)::sparse
!   integer(kind=int32),intent(in),optional::minsizenode
!  end subroutine
!#endif
!#if (_METIS==1)
!  !**GET ORDER
!  module function getordering_crs_64(sparse&
!                            ,ctype,iptype,rtype,compress,ccorder&
!                            ,pfactor,nseps,bglvl&
!                            ) result(perm)
!   class(crssparse_64),intent(in)::sparse
...
!#if (_SPAINV==1)
!  !**GET INCOMPLETE CHOLESKY FACTOR
!  module subroutine getichol_crs_64(sparse,minsizenode)
!   class(crssparse_64),intent(inout)::sparse
!   integer(kind=int32),intent(in),optional::minsizenode
!  end subroutine
!  !**GET SPARSE INVERSE
!  module subroutine getspainv_crs_64(sparse,minsizenode)
!   class(crssparse_64),intent(inout)::sparse
!   integer(kind=int32),intent(in),optional::minsizenode
!  end subroutine
!#endif
...
!  !**SAVE
!  module subroutine save_crs_64(sparse,namefile)
!   class(crssparse_64),intent(in)::sparse
!   character(len=*),intent(in)::namefile
!  end subroutine
  !**SCALE ALL ENTRIES
...
!  !**SOLVE WITH A TRIANGULAR FACTOR
!  module subroutine isolve_crs_64(sparse,x,y)
!   !sparse*x=y
!   class(crssparse_64),intent(in)::sparse
!   real(kind=wp),intent(out)::x(:)
!   real(kind=wp),intent(in)::y(:)
!  end subroutine
!  !**SOLVE WITH LDLt DECOMPOSITION
!  module subroutine solveldlt_crs_64(sparse,x,y)
!   !sparse*x=y
!   class(crssparse_64),intent(in)::sparse
!   real(kind=wp),intent(out)::x(:)
!   real(kind=wp),intent(in)::y(:)
!  end subroutine
 ...
 !> @brief Constructor; e.g., mat=crssparse_64(dim1,#elements,[dim2],[upper_storage],[output_unit])
 !! OR mat=crssparse_64('file',[output_unit])
 interface crssparse_64
  module procedure constructor_crs_64!,load_crs_64
 end interface
...
!FINAL
subroutine deallocate_scal_crs_64(sparse)
 type(crssparse_64),intent(inout)::sparse

 call sparse%destroy()

end subroutine

subroutine deallocate_rank1_crs_64(sparse)
 type(crssparse_64),intent(inout)::sparse(:)
