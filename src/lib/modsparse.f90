...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!CRS3!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!aaa
 ...
 type,extends(gen_sparse)::crs3sparse
  private
  ...
  procedure::multbym=>multgenm_csr3
  !> @brief Returns the number of non-zero elements
  procedure,public::nonzero=>totalnumberofelements_crs3
...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!CRS!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!aaa
 !> @brief Object for Compressed Row Storage
 type,extends(crs3sparse)::crssparse
  private
#if (_PARDISO==1)
...
!!**GET ELEMENTS
!function get_crs3(sparse,row,col) result(val)
! class(crs3sparse),intent(inout)::sparse
! integer(kind=int32),intent(in)::row,col
! real(kind=wp)::val
!
...
!**MULTIPLICATIONS
!subroutine multgenv_csr3(sparse,alpha,trans,x,val,y)
! !Computes y=val*y+alpha*sparse(tranposition)*x
! class(crs3sparse),intent(in)::sparse
! real(kind=wp),intent(in)::val,alpha
! real(kind=wp),intent(in)::x(:)
...
!end subroutine
!
!subroutine multgenm_csr3(sparse,alpha,trans,x,val,y)
! !Computes y=val*y+alpha*sparse(tranposition)*x
! class(crs3sparse),intent(in)::sparse
! real(kind=wp),intent(in)::val,alpha
! real(kind=wp),intent(in)::x(:,:)
...
!!**NUMBER OF ELEMENTS
!function totalnumberofelements_crs3(sparse) result(nel)
! class(crs3sparse),intent(in)::sparse
! integer(kind=int64)::nel
!
...
!!**PRINT
!subroutine print_crs3(sparse,lint,output)
! class(crs3sparse),intent(in)::sparse
! integer(kind=int32),intent(in),optional::output
! logical,intent(in),optional::lint
!
...
!end subroutine
!
!subroutine printsquare_crs3(sparse,output)
! class(crs3sparse),intent(inout)::sparse
! integer(kind=int32),intent(in),optional::output
!
...
subroutine deallocate_rank1_crs3(sparse)
 type(crs3sparse),intent(inout)::sparse(:)
